...
class Cli {
    ...
    inquirer
      ...
      .then((answers) => {
        ...
        if (selectedVehicle.vin === truck.vin) {
          // if it is, log that the truck cannot tow itself then perform actions on the truck
  ...
  performActions(): void {
    // Find the current selected vehicle
    const selectedVehicle = this.vehicles.find(
      vehicle => vehicle.vin === this.selectedVehicleVin
    );

    // If no vehicle is selected, return early
    if (!selectedVehicle) {
    ...
    // Define choices based on the vehicle type
    let actionChoices = [
      'Print details',
      'Start vehicle',
      'Accelerate 5 MPH',
    ...
    if (selectedVehicle instanceof Truck) {
      actionChoices.splice(8, 0, 'Tow a vehicle');
    }

    // Add wheelie option for motorbikes
    if (selectedVehicle instanceof Motorbike) {
      actionChoices.splice(8, 0, 'Do a wheelie');
    }
    ...
    inquirer
      ...
      .then((answers) => {
        ...
        if (answers.action === 'Print details') {
          ...
          // Find the truck and call findVehicleToTow
          for (let i = 0; i < this.vehicles.length; i++) {
          ...
          // Find the motorbike and do a wheelie
          for (let i = 0; i < this.vehicles.length; i++) {
